...
import { useSolanaCtx } from "../../hooks";

import {
  type CachedTokenBalance,
  useMintForAssetId,
  useSwapOutputTokensSolana,
  useSwapValidInputTokensSolana,
  useTokenBalances,
} from "./hooks";

export { CachedTokenBalance, useSwapOutputTokensSolana };
...
export type SwapContext = {
  ...
  // Token metadata
  fromTokens: CachedTokenBalance[];
  fromToken: CachedTokenBalance | undefined;
  toTokens: CachedTokenBalance[];
  toToken: CachedTokenBalance | undefined;
  // Amounts
...
export function SwapProvider({
  ...
  const [publicKeyOverride, setPublicKeyOverride] = useState(ctx.publicKey);
  ...
  const defaultFromTokenMint = useMintForAssetId(
    defaultFromAssetId ?? SOL_NATIVE_MINT
  );
  const { balances } = useTokenBalances({
    walletPublicKey: walletPublicKey.toBase58(),
    blockchain: Blockchain.SOLANA,
  });
  const [fromTokens, isLoadingFromTokens] =
    useSwapValidInputTokensSolana(balances);
  ...
  const fromToken = fromTokens.find((t) => t.token === fromMint);

  const [slippage, _] = useState(DEFAULT_SLIPPAGE_PERCENT);
  ...
  const [toTokens, isLoadingToTokens] = useSwapOutputTokensSolana(
    fromMint,
    balances
  );
  const toToken = toTokens.find((t) => t.tokenListEntry?.address === toMint);

  let availableForSwap = fromToken?.amount
    ? BigNumber.from(fromToken.amount)
    : Zero;
  ...
  const solanaToken = fromTokens.find((t) => t.token === SOL_NATIVE_MINT);
  const feeExceedsBalance =
    transactionFees?.total && solanaToken?.amount
      ? BigNumber.from(transactionFees.total).gt(solanaToken.amount)
      : undefined;
  ...
  const debouncedFromAmount = _useDebounce(fromAmount);

  useEffect(() => {
    const defaultFromMint = defaultFromTokenMint ?? SOL_NATIVE_MINT;
    const defaultToMint = defaultFromMint
      ? defaultFromMint === USDC_MINT.toString()
  ...
  const canSwitch =
    toToken?.token === WSOL_MINT || fromTokens.some((t) => t.token === toMint);
...
function _useDebounce(value: any, wait = DEFAULT_DEBOUNCE_DELAY) {
